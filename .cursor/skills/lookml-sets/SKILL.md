...
### 3. Excluding Fields (Preferred over `hidden: yes`)

Our best practice is to use sets at the Explore level to manage field visibility rather than using the `hidden: yes` parameter on individual fields. The `hidden: yes` parameter should only be used when explicitly requested.
