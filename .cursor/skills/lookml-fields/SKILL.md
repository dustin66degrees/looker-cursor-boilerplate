...
## 4. Visibility Best Practices

*   **Avoid `hidden: yes`**: Do not use the `hidden: yes` parameter on individual fields unless explicitly requested by the user.
*   **Use Field Sets**: Our best practice is to use field sets (`explore_fields`) at the Explore level to manage field visibility and curate the field picker. This provides a centralized and more maintainable approach to controlling what users see.
