...
4.  **Visibility**:
    -   Do not use `hidden: yes` for fields that should not be visible in the Explore (e.g., raw IDs, intermediate calculations).
    -   Fields that would be set with a `hidden: yes` will instead be omitted from the 
    `explore_fields` set on the view.
