  ...
  html:
    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
      ...
      {% if growth_rate._value > 0 %}
        <span style="color: green; font-size: 10px;">Γû▓ {{ growth_rate._rendered_value }}</span>
      {% elsif growth_rate._value < 0 %}
        <span style="color: red; font-size: 10px;">Γû╝ {{ growth_rate._rendered_value }}</span>
      {% endif %}
    </div>
  ;;
}
